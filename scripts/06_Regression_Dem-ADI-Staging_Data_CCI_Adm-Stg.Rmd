...
```{r Full model of CCI}
# Full model of CCI
m_CCI_full <- lm(cci_total ~ age + sex + smoking + marital_status + language + insurance_type + pat_race + ethnicity + ADI, data = df)
...
m_adm_stg_full <- lm(admission_stage ~ age + sex + smoking + marital_status + language + insurance_type + pat_race + ethnicity + ADI + cci_total, data = df)
...
tab_model(m_CCI_full, m_adm_stg_full,
          ...
          #show.reflvl = T,
          pred.labels = c("Intercept", "Age", "Male", "Non-Smoker", "Unknown Smoker", "Marital Status: Paired", "Marital Status: Unknown/Declined", "Not English", "Private Insurance", "Asian", "Black", "White", "Unknown/Declined Race", "Hispanic/Latino", "Not Hispanic", "Area Deprivation Index (ADI)", "Charlson Comorbidity Index (CCI)"),
